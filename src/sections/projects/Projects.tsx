...
export default function Projects() {
  ...
  const { t } = useApp();
  const projTranslations = t.projects.items;
  ...
  return (
    <section id="projects" className="projects section">
      <div className="container">
        ...
        <div ref={ref as React.RefObject<HTMLDivElement>} className={`projects-grid fade-up ${isVisible ? 'visible' : ''}`}>
          {projects.map((project, i) => {
            const pt = projTranslations[i];
            return (
              <article key={i} className="project-card">
                ...
                <h3 className="project-title">{pt?.title ?? project.title}</h3>
                <p className="project-desc">{pt?.description ?? project.description}</p>
